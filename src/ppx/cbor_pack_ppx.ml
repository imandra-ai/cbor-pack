...
let deser_key_name_of_ty_name (ty_name : string) : string =
  if ty_name = "t" then
    "deser_cyclic_key"
  else
    "deser_cyclic_key_" ^ ty_name

(* name for variables *)
let name_poly_var_ v = spf "_cbpack_poly_%s" v

let deser_key_name_of_ty (ty : type_declaration) : string =
  deser_key_name_of_ty_name ty.ptype_name.txt
...
let deser_key_name_of_lid = map_lid ~f:deser_key_name_of_ty_name
...
let attr_ser =
  Attribute.declare "cbpack.ser" Attribute.Context.core_type
    Ast_pattern.(single_expr_payload __)
    (fun x -> x)

(* attribute to force a [string] to encode into CBOR bytes *)
let attr_use_bytes =
  Attribute.declare "cbpack.use_bytes" Attribute.Context.core_type
    Ast_pattern.(pstr nil)
    ()

(* field in record: use this key *)
let attr_use_field_name =
  Attribute.declare "cbpack.key" Attribute.Context.label_declaration
    Ast_pattern.(single_expr_payload @@ estring __)
    (fun x -> x)

(* cstor in sum type: use this key *)
let attr_use_cstor_name =
  Attribute.declare "cbpack.cstor" Attribute.Context.constructor_declaration
    Ast_pattern.(single_expr_payload @@ estring __)
    (fun x -> x)

(* mutable field in record: placeholder for cyclic value *)
let attr_placeholder =
  Attribute.declare "cbpack.placeholder" Attribute.Context.label_declaration
    Ast_pattern.(single_expr_payload __)
    (fun x -> x)

(* use name for all fields in record *)
let attr_use_field_names =
  Attribute.declare "cbpack.use_field_names" Attribute.Context.type_declaration
    Ast_pattern.(pstr nil)
    ()
...
let rec ser_expr_of_ty (e : expression) ~(ty : core_type) : expression =
  ...
  | [%type: int] -> by_basic_ser [%expr Cbor_pack.Ser.int]
  | [%type: int32] -> [%expr Cbor_pack.Ser.int (Int32.to_int [%e e])]
  | [%type: int64] ->
    [%expr
      let i = [%e e] in
      if Int64.(of_int (to_int i) = i) then
        Cbor_pack.Ser.int (Int64.to_int i)
      else
        Cbor_pack.Ser.string (Int64.to_string i)]
  | [%type: nativeint] ->
    [%expr
      let i = [%e e] in
      if Nativeint.(of_int (to_int i) = i) then
        Cbor_pack.Ser.int (Nativeint.to_int i)
      else
        Cbor_pack.Ser.string (Nativeint.to_string i)]
  | [%type: string] ->
    if is_some_ @@ Attribute.get ~mark_as_seen:false attr_use_bytes ty then
      [%expr Cbor_pack.Ser.add_bytes ser (Bytes.unsafe_of_string [%e e])]
    else
      [%expr Cbor_pack.Ser.add_string ser [%e e]]
  | [%type: bytes] -> [%expr Cbor_pack.Ser.add_bytes ser [%e e]]
  | [%type: bool] -> by_basic_ser [%expr Cbor_pack.Ser.bool]
  | [%type: char] ->
    by_basic_ser [%expr fun c -> Cbor_pack.Ser.int (Char.code c)]
  | [%type: unit] -> by_basic_ser [%expr fun _ -> Cbor_pack.Ser.unit]
  | [%type: float] -> by_basic_ser [%expr Cbor_pack.Ser.float]
  | [%type: [%t? ty_arg0] option] ->
    [%expr
      match [%e e] with
      | None -> Cbor_pack.Ser.(list [])
      | Some x ->
        let x = [%e ser_expr_of_ty [%expr x] ~ty:ty_arg0] in
        Cbor_pack.Ser.(list [ x ])]
  | [%type: [%t? ty_arg0] list] ->
    [%expr
  ...
  | { ptyp_desc = Ptyp_tuple args; ptyp_loc = loc; _ } ->
    ...
    let vbs =
      ...
      [ A.Vb.mk tup_pat e ]
    in
    let body = [%expr Cbor_pack.Ser.(list [%e mk_list ~loc ser_args])] in
...
let rec deser_expr_of_ty (e : expression) ~(ty : core_type) : expression =
  ...
  | [%type: int] -> by_full_deser [%expr Cbor_pack.Deser.to_int]
  | [%type: int32] ->
    [%expr Cbor_pack.Deser.to_int deser [%e e] |> Int32.of_int]
  | [%type: int64] ->
    [%expr
      match Cbor_pack.Deser.deref_if_ptr deser [%e e] with
      | `Int i -> Int64.of_int i
      | `Text s ->
        (try Int64.of_string s
         with _ -> raise (Cbor_pack.Deser.Error "expected int64"))
      | _ -> raise (Cbor_pack.Deser.Error "expected int64")]
  | [%type: nativeint] ->
    [%expr
      match Cbor_pack.Deser.deref_if_ptr deser [%e e] with
      ...
      | `Text s ->
        (try Nativeint.of_string s
         with _ -> raise (Cbor_pack.Deser.Error "expected nativeint"))
      | _ -> raise (Cbor_pack.Deser.Error "expected nativeint")]
  | [%type: string] ->
    if is_some_ @@ Attribute.get ~mark_as_seen:false attr_use_bytes ty then
      [%expr Bytes.unsafe_to_string @@ Cbor_pack.Deser.to_bytes deser @@ [%e e]]
    else
      by_full_deser [%expr Cbor_pack.Deser.to_text]
  | [%type: bytes] -> by_full_deser [%expr Cbor_pack.Deser.to_bytes]
  | [%type: bool] -> by_full_deser [%expr Cbor_pack.Deser.to_bool]
  | [%type: char] -> [%expr Char.chr (Cbor_pack.Deser.to_int deser [%e e])]
  | [%type: unit] -> by_full_deser [%expr Cbor_pack.Deser.to_unit]
  | [%type: float] -> by_full_deser [%expr Cbor_pack.Deser.to_float]
  | [%type: [%t? ty_arg0] option] ->
    [%expr
      match Cbor_pack.Deser.(to_list deser [%e e]) with
      | [] -> None
      | [ x ] ->
        let x = [%e deser_expr_of_ty [%expr x] ~ty:ty_arg0] in
        Some x
      | _ ->
        raise (Cbor_pack.Deser.Error "expected an option (list of 0 or 1 elt)")]
  | [%type: [%t? ty_arg0] list] ->
    [%expr
      let l = Cbor_pack.Deser.(to_list deser [%e e]) in
      List.map (fun x -> [%e deser_expr_of_ty [%expr x] ~ty:ty_arg0]) l]
  | [%type: [%t? ty_arg0] array] ->
    [%expr
      let a = Array.of_list @@ Cbor_pack.Deser.to_list deser [%e e] in
  ...
  | { ptyp_desc = Ptyp_tuple args; ptyp_loc = loc; _ } ->
    ...
    let br_good =
      ...
      A.Exp.case [%pat? `Array [%p lhs] as _p] rhs
    and br_bad1 =
      let msg =
        A.Const.string
      ...
      A.Exp.case
        [%pat? `Array _]
        [%expr raise (Cbor_pack.Deser.Error [%e A.Exp.constant msg])]
    and br_bad2 =
      A.Exp.case
        [%pat? _]
        [%expr raise (Cbor_pack.Deser.Error "expected tuple")]
    in
    A.Exp.match_ ~loc
      [%expr Cbor_pack.Deser.deref_if_ptr deser [%e e]]
...
let ser_expr_of_tydecl ~cyclic (decl : type_declaration) : expression =
  let loc = decl.ptype_loc in
  let self = A.Exp.ident @@ lid ~loc "self" in

  let hashcons =
    let b =
      not @@ is_some_ (Attribute.get ~mark_as_seen:true attr_nohashcons decl)
    in
    if b then
      [%expr true]
    else
  ...
  let body =
    ...
    | Ptype_variant cstors ->
      let ser_cstor (i : int)
        ...
        let cstor_key =
          ...
          | None ->
            [%expr Cbor_pack.Ser.int [%e A.Exp.constant (A.Const.int i)]]
          | Some s ->
    ...
    | Ptype_record labels ->
      ...
      let e =
        labels
        |> List.mapi (fun i ({ pld_name = field_name; pld_type; _ } as fld) ->
               ...
               let key =
                 ...
                 with
                 | Some k ->
                   [%expr `Text [%e A.Exp.constant (A.Const.string k)]]
                 | None when use_field_names ->
                   [%expr
                     `Text [%e A.Exp.constant (A.Const.string field_name.txt)]]
                 | None -> [%expr `Int [%e A.Exp.constant (A.Const.int i)]]
               in
               [%expr [%e key], [%e e]])
        |> mk_list ~loc
      in
  ...
  if cyclic then
    [%expr
      fun ser self ->
        ...
        with_cycle_detection ser self @@ fun self ->
        (* be sure to add entry inside the memo function *)
...
let deser_expr_of_tydecl ~cyclic (decl : type_declaration) : expression =
  let loc = decl.ptype_loc in
  let self = A.Exp.ident @@ lid ~loc "self" in
  ...
  let body =
    ...
    | Ptype_open -> [%expr [%error "cannot derive cbpack for open type"]]
    | Ptype_variant cstors ->
      let deser_cstor (i : int)
          ({ pcd_args; pcd_name = cname; pcd_loc = loc; _ } as cstor) : case =
        (* constructor identifier *)
        let cstor_key =
          match Attribute.get ~mark_as_seen:true attr_use_cstor_name cstor with
          | None -> [%pat? `Int [%p A.Pat.constant (A.Const.int i)]]
          | Some s -> [%pat? `Text [%p A.Pat.constant (A.Const.string s)]]
        in
        ...
        let lhs, rhs =
          ...
          | Pcstr_tuple [] ->
            ...
            lhs, rhs
          | Pcstr_tuple [ ty0 ] ->
            let lhs = [%pat? `Array [ [%p cstor_key]; p ]] in
            let rhs =
              [%expr
            ...
            let lhs =
              let args =
                ...
                |> List.mapi (fun i ty ->
                       ...
                       A.Pat.var { loc; txt = spf "x_%d" i })
              in
              [%pat? `Array [%p mk_list_pat ~loc (cstor_key :: args)]]
            in
            let rhs =
              let deser_fields =
                l
      ...
      let fail =
        ...
        and rhs =
          let err =
            A.Exp.constant
              (A.Const.string @@ spf "expected %s" decl.ptype_name.txt)
          in
          [%expr raise (Cbor_pack.Deser.Error [%e err])]
        in
        B.case ~lhs ~rhs ~guard:None
      in
      let branches = List.mapi deser_cstor cstors @ [ fail ] in
      [%expr
        let e = Cbor_pack.Deser.deref_if_ptr deser [%e self] in
        [%e A.Exp.match_ [%expr e] branches]]
    | Ptype_record labels ->
      let fields =
        labels
        |> List.mapi
             (fun i ({ pld_name = field_name; pld_type; pld_mutable; _ } as fld)
             ->
               let key =
                 match
                   Attribute.get ~mark_as_seen:true attr_use_field_name fld
               ...
               let field_of_self =
                 [%expr Cbor_pack.Deser.map_entry deser ~k:[%e key] self]
               in
               let value_direct = deser_expr_of_ty field_of_self ~ty:pld_type in

               let value =
                 match
                   Attribute.get ~mark_as_seen:true attr_placeholder fld
                 with
                 | None -> value_direct
                 | Some p ->
                   (* placeholder: this must be a mutable field holding
                      a type in this cyclic clique. We produce code
                      that fills the field with the placeholder, and
                      delays the actual lookup of the value to later. *)
                   if pld_mutable <> Mutable then
                     [%expr [%error "placeholder only works on mutable fields"]]
                   else if not cyclic then
                     [%expr [%error "placeholder only works for cyclic types"]]
                   else (
                     (* the type must be a constructor type *)
                     match pld_type with
                     | { ptyp_desc = Ptyp_constr (ty_lid, _args); _ } ->
                       let key =
                         let txt = deser_key_name_of_lid ty_lid.txt in
                         A.Exp.ident { loc = ty_lid.loc; txt }
                       in
                       [%expr
                         let fself = [%e field_of_self] in
                         (*Format.printf "lookup for %s %a@." (Cbor_pack.Deser.name_of_key [%e key])
                           Cbor_pack.pp_diagnostic self;*)
                         let lazy_v =
                           match
                             Cbor_pack.Deser.find_key deser ~key:[%e key] fself
                           with
                           | Some v ->
                             (* Format.printf "found@.";*)
                             v
                           | None ->
                             (*Format.printf "miss for %s@." (Cbor_pack.Deser.name_of_key [%e key]);*)
                             (* Format.printf "else: %B@." (Option.is_some @@ Cbor_pack.Deser.find_key deser ~key:[%e key] self); *)
                             lazy [%e value_direct]
                         in
                         (* set value after deser is done *)
                         Cbor_pack.Deser.delay deser (fun () ->
                             (*Format.printf "delayed set field@.";*)
                             [%e
                               A.Exp.setfield [%expr Lazy.force _lazy_result]
                                 (lid_of_str field_name)
                                 [%expr Lazy.force lazy_v]]);
                         (* return placeholder *)
                         [%e p]]
                     | _ ->
                       [%expr
                         [%error "placeholder only works for a cyclic type"]]
                   )
               in
  ...
  if cyclic then (
    let key =
      let txt = deser_key_name_of_ty decl in
      A.Exp.ident @@ lid_of_str { loc; txt }
    in
    [%expr
      fun deser self ->
        let open Cbor_pack.Deser in
        (* use cycle detection. Adds [_lazy_result] in scope. *)
        with_cycle_detection ~key:[%e key]
          (fun _lazy_result deser self -> [%e body])
          deser self]
  ) else
...
let generate_impl_ (rec_flag, type_declarations) =
  ...
  let ser_decls =
    List.map
      (fun ty ->
        ...
        let def = fun_poly_gen_ ~loc ty @@ ser_expr_of_tydecl ~cyclic ty in
  ...
  let deser_decls =
    List.map
      (fun ty ->
        ...
        let def = fun_poly_gen_ ~loc ty @@ deser_expr_of_tydecl ~cyclic ty in
        A.Vb.mk (A.Pat.var { loc; txt = fname }) def)
      type_declarations
  in
    ...
    if cyclic then (
      let decls =
        List.map
          (fun ty ->
            let loc = ty.ptype_loc in
            let fname = deser_key_name_of_ty ty in
            let rhs =
              if ty.ptype_params <> [] then
                [%expr
                  [%error "cbpack.cyclic only works for non polymorphic types"]]
              else
                [%(* use explicit type to prevent "weak type" warnings if we
                     end up not using this key *)
                  expr
                  (Cbor_pack.Deser.make_key
                     ~name:
                       [%e A.Exp.constant (A.Const.string ty.ptype_name.txt)]
                     ()
                    : [%t A.Typ.constr (lid_of_str ty.ptype_name) []]
                      Cbor_pack.Deser.key)]
            in
            A.Vb.mk (A.Pat.var { loc; txt = fname }) rhs)
          type_declarations
      in
  ...
  bracket_warn
    (List.flatten
       [ [ ser_defs ]; key_defs; [ deser_defs ]; deser_defs_wrappers ])
  ...
  let values =
    List.map
      (fun ty ->
        ...
        let poly_sers =
          param_names ty
          |> List.map (fun n -> [%type: [%t A.Typ.var n] Cbor_pack.Ser.t])
        and poly_desers =
          param_names ty
          |> List.map (fun n -> [%type: [%t A.Typ.var n] Cbor_pack.Deser.t])
        in
        ...
        let decl_ser =
          let ty =
            mk_arrow ~loc poly_sers @@ [%type: [%t tye] Cbor_pack.Ser.t]
          in
          A.Val.mk { loc; txt = name_ser } ty
        and decl_deser =
          let ty =
            mk_arrow ~loc poly_desers @@ [%type: [%t tye] Cbor_pack.Deser.t]
          in
...
let myderiver =
  Deriving.add "cbpack" ~sig_type_decl:intf_generator
