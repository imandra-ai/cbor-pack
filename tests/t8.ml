...
let rec ser_tree st (t : tree) : CP.cbor =
  match t with
  | N foo ->
    CP.Ser.(
      list [ string "N"; CP.Ser.with_cache key_ser_foo foo_to_cbpack st foo ])
  | T (r1, r2) ->
    CP.Ser.(list [ string "T"; ser_tree_ref st r1; ser_tree_ref st r2 ])

and ser_tree_ref st (r : tree_ref) : CP.cbor =
  let ser_uncached st r = CP.Ser.(list [ int r.id; ser_tree st r.tree ]) in
  CP.Ser.with_cache key_ser_tree_ref ser_uncached st r

let rec deser_tree st (c : CP.cbor) : tree =
  let l = CP.Deser.to_list st c in
  match l with
  | [ `Text "N"; foo ] ->
    N (CP.Deser.with_cache key_deser_foo foo_of_cbpack st foo)
  | [ `Text "T"; t1; t2 ] ->
...
and deser_tree_ref st c : tree_ref =
  let deser_uncached st c =
    ...
    | [ `Int id; t ] -> { id; tree = deser_tree st t }
...
let cbor_big_tree = CP.to_cbor ser_tree_ref big_tree
let () = Format.printf "ser(big_tree): %a@." CP.pp_diagnostic cbor_big_tree

let () =
  Format.printf "size: %d@." (CBOR.Simple.encode cbor_big_tree |> String.length)

let () =
  Format.printf "marshal size: %d@."
    (Marshal.to_string big_tree [] |> String.length)

let big_tree2 = CP.of_cbor_exn deser_tree_ref cbor_big_tree
